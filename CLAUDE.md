...
**Not yet retired (Task 13, blocked on two human verifications — see below):** `public/ref/*.wav` (120 files) and `public/ref/manifest.json` are still git-tracked and still what `src/dev/make-clips.ts` writes; `api/upload.ts`, `api/auth.ts`, `api/_passcode.ts` and `@vercel/blob` are still live and still what the pre-migration booth flow used; `src/dev/pull-recordings.ts` is still present and registered in `package.json`; `src/dev/manifest.test.ts` is still present and registered in the tsconfigs; `src/record/wordlist.ts`'s `WORDS` literal is still exported. None of this is reachable from a live player path any more (the booth talks to the Worker now, the game reads the DB/fallback, not the manifest) but none of it has been deleted, because the retirement's own precondition hasn't been met: a full run played off R2 in production (`VITE_CLIPS_BASE_URL` is not yet set in Vercel) and the booth recording a new word end to end. Do not delete any of it until both have happened and Task 13 runs.
