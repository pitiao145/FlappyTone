...
- **Three tiers, two gates, and gating is live.** `guest` (anonymous/signed-out): 3 runs/day, local-only progress, no per-tone visualiser practice, zero words. `free` (email account): 10 runs/day, saved+synced progress, a real leaderboard row under its generated name, per-tone visualiser practice, first 5 words/tone. `pro` (paid): unlimited runs, every word, full leaderboard + chosen name, customization. Gate 1 (guest→free) is runs-cap + persistence; Gate 2 (free→pro) is depth/content/customization, **not** run quantity.
- **`src/game/tiers.ts` holds `TIER_LIMITS`**, the single source of truth for per-tier limits (`runsPerDay`, `wordsPerTone`, `visualiserPerTone`, `leaderboardFull`, `customization`). Deliberately free of `src/data/` imports, so game code and the UI can read limits without pulling in the Supabase graph. `plan.ts`'s pricing copy, `dailyLimit.ts`'s cap, `words.ts`'s slice, `Leaderboard.tsx`'s teaser cut, and `tier.ts`'s resolution all read it now — nothing here restates a number.
- **Words are sliced by a parameter, not a lookup inside `words.ts`.** `wordsOfTone(words, tone, limit)` takes the cap as an argument; `words.ts` itself has no `src/data/`/tier import. `pickWord` (feeding a scored run) always calls it with no limit — a guest's zero-word visualiser cap must never starve their actual game. Only the visualiser's per-tone practice list passes `TIER_LIMITS[tier].wordsPerTone`.
- **The run cap is server-authoritative for accounts, device-local for guests — not the same enforcement for both, deliberately.** `dailyLimit.ts` still owns the local counter (unchanged shape, still not tamper-proof), but a signed-in player's run also POSTs to `api/run.ts`, the sole writer of `daily_runs` (migrations `0010`/`0011`, permanent accounts only — an anonymous session is rejected). A guest stays local-only: clearing storage mints a new anonymous identity, so a server row would count against nothing durable. The client sends its own local date; the server bounds it to ±1 day of its own UTC clock rather than trusting it outright. A failed call to `api/run.ts` never blocks the run that's already starting — local already incremented, and the server just doesn't hear about it yet. This means the "server-enforced daily limits" line in Out of Scope below is now only half true; see there.
...
- **`has_access` is a server-written flag on its own `entitlements` table, never a `profiles` column.** `src/data/entitlements.ts` only reads it. `entitlements` is select-own with **no client write policy at all** — the same enforcement shape as `leaderboard_scores`. A column on `profiles` would have been client-flippable in one devtools call, since `profiles` is client-writable under RLS. `api/webhook-ls.ts` (Lemon Squeezy) is the only writer.
- **Payment: `api/webhook-ls.ts` is the only writer of `has_access`**, service-role key like `api/score.ts`. It verifies Lemon Squeezy's HMAC-SHA256 signature over the *raw* body, timing-safe, **before** parsing a single byte — an unverified webhook is a public "make me Pro" button. The buyer is identified strictly by `meta.custom_data.user_id`, carried through the checkout link (`EarlyBirdModal.tsx`'s `buildCheckoutUrl`) — never by the order's email, which the buyer types and could point anywhere. Writes are absolute state (`has_access = true/false`), not a toggle or increment, so a retried or out-of-order webhook delivery converges instead of drifting. The Pay button requires a signed-in account (a purchase needs a `user_id` to attach to); with no checkout URL configured it falls back to the old disabled-button-plus-email-capture behaviour. `src/data/purchaseReturn.ts` polls `refreshTier()` for ~10s after `?purchased=1` and only ever resolves to "confirmed" or "still syncing" — never "you're not Pro," since the webhook can legitimately land after the browser is already back.
- **The leaderboard is tier-gated on read, not just on write.** A guest sees the board and their would-be rank but cannot join (`api/score.ts` 403s an anonymous session — see below). A free account joins under its generated name. `Leaderboard.tsx` truncates the visible list to `TEASER_ROWS` (10) for anyone without `leaderboardFull`, always pinning the player's own row below the cut.
- **`api/score.ts` was open to anonymous writes until 8 Sep 2026** — it verified the JWT but never checked `is_anonymous`, so anyone could mint an anonymous session from devtools and post a real leaderboard row; `GameOver.tsx`'s `canJoin` was only a UI courtesy. Now a 403. See DECISIONS.md — this is logged as a real incident, not a routine tidy-up.
- **Anonymous sign-in (`signInAnonymously()`) is now unreachable from any live path**, since joining the board — its only caller — no longer signs a guest in first. Existing anonymous rows from before this change were left alone.
- **Stats sync also fires after each run and on app load**, both fire-and-forget, in addition to sign-in — not just the two triggers Phase 2 shipped with. Still local-first: the UI always renders from `localStorage`, so a dead network degrades to "your stats still work," not an error.
...
- **`src/game/runHistory.ts` and `src/game/dailyLimit.ts`** (27 Aug 2026): the Progress/Profile tabs need real, device-local stats — lifetime run/gate/word counts, the last 5 runs' per-tone accuracy, a "N of 5 free runs today" counter — to avoid faking numbers the UI claims are real. Both follow `settings.ts`'s key/version/validate convention and store nothing that leaves the device. `dailyLimit.ts`'s local counter is explicitly **not tamper-proof** — its checksum only deters a casual devtools edit. That was written before accounts existed; the counter itself hasn't moved, but a signed-in player's count is now also checked server-side (`api/run.ts`, see the tiers bullet above) — the local copy stays for guests and for showing a number without a network round trip. Don't build product logic that assumes the local half can't be bypassed, and don't reach for this pattern beyond what the free-tier teaser needs.
- **`api/*.ts`**: four small Vercel functions. `api/newsletter.ts` proxies a landing-page email signup to ConvertKit so the client never sees the API key. `api/score.ts`, `api/run.ts` and `api/webhook-ls.ts` are the three gameplay/account functions: the sole writers of the leaderboard, the server run count, and `has_access` respectively — each holds the service-role key and computes or verifies server-side the one value the client must not be trusted to supply. Apart from them, the player-facing game talks to nothing but Supabase reads and PostHog. **A test file in `api/` must be named `_*.test.ts`** — without the underscore Vercel deploys it as a public endpoint, which `api/_imports.test.ts` checks. (The passcode-gated `/record` booth upload — `api/upload.ts` + `api/auth.ts` + `api/_passcode.ts` + `@vercel/blob` — was retired in Task 13, Sep 2026: the booth talks to the clips Worker's `/raw` and `/booth/words` routes now. `_json.ts`, the one thing `_passcode.ts` held that the other four functions still needed, moved to its own file.)
...
api/          Vercel functions: newsletter signup, and score.ts/run.ts/webhook-ls.ts (the leaderboard, the server run count, and has_access — each the sole writer of its value).
...
Clip-pipeline and demo-clip scripts (`process-clips`, `import-words`, `update-demo`, etc.) are covered where they're used below, not repeated here — `npm run` with no args lists everything in `package.json`.
...
**Status: migrated to a Supabase `words` table + Cloudflare R2/Worker pipeline (audio-migration branch, Sep 2026), per `docs/SPECS/flappytone-SPEC-clip-catalog-r2.md`. The plan's own Decisions 1, 2, 6, 7, 9 were overridden during the build — see DECISIONS.md "Clip catalog: DB + R2 migration" for why. Task 13 (retiring the pre-migration path) landed Sep 2026 — the manifest/Blob/passcode path described in earlier drafts of this section is gone; this describes what's true now.**

**The pipeline, current shape:** `npm run import-words` writes a TSV into the `words` table → Jane records at `/record`, which talks to the Worker's `/raw` and `/booth/words` routes (the booth opens on an overview of what's pending vs. recorded and arms the mic only when she asks — `src/record/boothArming.ts` holds the three arming rules and the overwrite incident behind them; re-recording a `published` word needs a confirm) → `npm run process-clips -- --speaker <id>` measures the take (same `src/dev/clipCut.ts` as before, byte-identical), uploads the finished clip to the private `flappytone-clips` R2 bucket at `clips/{speaker}/{id}.wav`, and upserts `duration_s`/`onset_s`/`clip_s`/`polyline`/`contour` onto that speaker's `word_clips` row → `npm run export-fallback` snapshots the **default speaker's** published catalog into `src/data/wordsFallback.json`, the bundle's offline fallback.

**`--speaker` is required and is validated against the `speakers` table; an unknown id exits non-zero rather than defaulting.** Every measurement `process-clips` makes — the pitch-search seed, the session reference, the cohort chao map, `clipReview`'s duration medians — is a property of one voice, so a forgotten argument would normalise one speaker's cohort against another's map and write the result over their rows. `speakers.f0_seed` supplies the seed (Jane's row holds exactly `168`); an existing `clip_key` is kept verbatim, so Jane's pre-roster objects stay where they are and a re-cut is never a bulk R2 move. `verify-clips` takes `--speaker` too, or walks every active speaker and reports per speaker rather than pooling the totals.

**`export-fallback` bundles the default speaker only** (`speakers.is_default`), and writes no `exportedAt` — the bundle is the dead-network path, and a dead network means the clips Worker is unreachable too, so no clip audio plays whichever voice was chosen; a second voice's rows would be weight on the landing page's critical path describing geometry nobody can hear. The timestamp went because the file's whole job is to be diffable: a re-export with no data change must produce an empty `git diff`, which is what proves a pipeline refactor moved no measurement.
...
**The booth's speaker is derived from its passcode, server-side, and no booth route accepts a speaker from the client.** The Worker's `resolveSpeaker` (`workers/clips/src/passcode.ts`) maps the `x-record-passcode` header to a `speakers.id` through one JSON secret, `RECORD_PASSCODES` (`{"<code>":"jane", …}`) — so adding a recorder is a secret update, not a code change. It returns a discriminated union rather than `Response | null`, so a route that forgets the denial branch does not compile, and it compares against **every** entry with no early exit, so response time reveals neither how many codes exist nor which prefix matched. Unset, unparseable, or a speaker slug the DB could not hold all 503 — fail-closed, as before. `/booth/words` returns only that speaker's pending/recorded split (scoped by the query, not filtered client-side) plus their display name, counting a word with no `word_clips` row for them as `pending`; `/raw` writes `raw/{speaker}/{session}/{id}.wav` and upserts `word_clips` on an explicit `(word_id, speaker_id)` conflict target, never an update keyed on `word_id` alone. **This retires a class of accident rather than guarding against it:** a male session physically cannot touch Jane's row, which is exactly what the `ma1b` overwrite did. It is a second, independent guard — `boothArming.ts`'s rules (only the bulk pass self-arms, a `published` word asks first, a take that replaced a published clip stops the run) are unchanged and still load-bearing.

**Clip audio is gated behind a play ticket, and guests get clips too.** `POST /token` on the Worker (`clips.flappytone.com`, source in `workers/clips/`) verifies a Supabase session JWT if one is sent — **ES256 against the project's JWKS, not HS256 against a shared secret** (Decision 1 override: Supabase signs asymmetrically; there is no `SUPABASE_JWT_SECRET`) — and mints a short-lived (30 min), IP-bound HS256 ticket carrying a `tier` (`guest`/`free`/`pro`). It **never hard-fails**: a missing, malformed, expired or anonymous-session JWT all resolve to a `guest` ticket rather than a 401 (Decision 2 override — the spec's "no JWT ⇒ no clips" was dropped; clips are what makes the game playable at all). `GET /clip/:id` then checks the ticket and, only for a `min_tier='pro'` word, the ticket's tier. **Protection against bulk download is defence-in-depth, not a hard wall:** private buckets (no `r2.dev`/public domain/CORS on either), no listing endpoint, one clip per request, 30-minute IP-bound tickets, and a Cloudflare WAF rate-limit rule on `/clip/*`, `/token`, and (tighter) the booth's `/auth` and `/booth/*` — **the rate-limit rule itself is not yet configured**, so the booth passcode has no rate limit today (blocked on Pierre's Cloudflare dashboard step; see `docs/SPECS/R2_SETUP.md`).

**`min_tier` (game access) and `TIER_LIMITS.wordsPerTone` (visualiser practice depth) are two different gates — keep them that way.** They were tangled once: the spec's per-tier word split (first N words/tone free, rest `pro`) was implemented faithfully but contradicted an invariant this file already stated — "a guest's zero-word visualiser cap must never starve their actual game" — so the scored game played a synthetic sweep for ~83% of cues for every non-Pro player. Logged as a real incident in DECISIONS.md, not a tidy-up. Fixed: every word's `min_tier` is `'free'` today; the run's word pool and `/clip`'s gate read `min_tier`, the visualiser's practice-list slice reads `TIER_LIMITS[tier].wordsPerTone` (unchanged, still `0/5/all`). `src/dev/import-words.ts` (which is what actually sets the default on a new row, not `process-clips.ts` — the latter's own UPDATE is narrow and never touches `min_tier`) must keep defaulting new words to `min_tier='free'` — do not reintroduce the first-N-per-tone rule.
...
**The catalog is a roster now: `words` + `speakers` + `word_clips`.** A word is who it is (`words`: hanzi, pinyin, tone, position, `min_tier`); a *recording* of it is a row in `word_clips`, keyed `(word_id, speaker_id)`, and every measurement lives there — `clip_key`, `duration_s`, `onset_s`, `clip_s`, `polyline`, `contour`, `status`, `updated_at`. `speakers` holds the voices themselves, with attributes (`gender`, `accent`, `is_default`, `active`) rather than a `voice` enum — see DECISIONS.md for why the player's stored setting is an axis and never a speaker id. **`min_tier` stayed on `words`**: it is game access, a property of the word, not of who read it.

**The clip route is `/clip/:speaker/:id`, and the speaker is a path segment on purpose.** It is part of the edge cache key. A cache key without the speaker serves one voice's audio under the other's name to everyone downstream, with no error anywhere and nothing for a player to notice — which is exactly the failure a query parameter invites, since a key is easy to build from the path alone. The single-segment `/clip/:id` back-compat route was removed rather than carried forward; no real user has ever run this architecture. `?v=` is the clip's own `word_clips.updated_at`, so re-recording one word busts that clip's cache and nothing else's.

Seven rules that hold the pipeline together, restated against `process-clips` (formerly against `make-clips`) — see DECISIONS.md for the incidents behind each:

1. **`src/dev/clipCut.ts` is the only measurement**, shared by `make-ref-clips` (the four anchors) and `process-clips` (everything Jane records, via `src/dev/clipPipeline.ts`). After touching it, regenerate and check `git diff fixtures/anchors` is empty and the catalog rows' `polyline`/`contour` didn't move (`npm run process-clips -- --speaker <id> --dry-run` diffs before writing).
...
3. **`clipReview.ts` flags, it never blocks — and every comparison it makes is within one voice.** Its tests assert Jane's four anchor clips pass clean. The tone-duration medians come from that speaker's own published `word_clips` rows; measured against another's they produce complaints that are individually true and jointly useless (a male take flagged "276ms against a tone-2 median of 1050ms" *and* "f0Center is probably wrong for this speaker" — both right, both because the numbers were Jane's).
4. **`f0Center` is measured per session, not read from `speakers.json`.** Pitch drifts between sittings; `measurePitchReference` pools the session's own voiced frames. Exception: the pipeline's pitch-search *seed*, which is **per speaker but never derived from the catalog** — it comes from `speakers.f0_seed` through `resolveSeed` in `src/dev/clipPipeline.ts`, whose default and exported constant `SEED_F0_CENTER` stays `168`, and Jane's row holds exactly `168` so her 120 corridors are unmoved. Seeding from the last published word's own reference moved 90 of 120 polylines in the 3rd decimal, which is why the value is a roster column and not something recomputed. Guarded by a golden `cutClip` test over all four anchors, which `clipPipeline.test.ts` also proves still bites by re-cutting them at another speaker's seed.
5. **`word_clips` is per speaker, and so is `f0_seed`.** A measurement belongs to a recording, never to a word: two voices reading the same hanzi produce two polylines, two onsets and two clip lengths, and the corridor a player flies has to be the one measured from the clip they just heard. Never write a measurement onto `words`, and never reuse one speaker's `f0_seed` to measure another's take — the pitch search would be seeded off the wrong range.
6. **A voice is added as rows, not as a schema change.** A second accent or a third speaker is an `INSERT` into `speakers` plus that speaker's `word_clips`. If adding a voice needs a migration, the shape is wrong.
7. **`src/data/catalogSeam.test.ts` is the seam between the pipeline and the game** — successor to `manifest.test.ts` (invariant unchanged, target moved; `manifest.test.ts` itself was deleted in Task 13). It pins `CATALOG_SELECT`/`wordsFromCatalog`/`FALLBACK_COLUMNS` together: a renamed DB column still surfaces as an empty inventory, which still silently degrades to tuning defaults and looks exactly like the game working.
...
1. **`src/analytics/session.ts`'s `AnalyticsEvent` union is the only property-level gate on a gameplay event — `before_send` filters PostHog's defaults, not ours.** Be exact here, because the weaker reading is a false privacy guarantee: `sanitizeGameProperties` matches the event *name* against `GAME_EVENTS`, then drops `$`-prefixed keys except the two country-level geo fields, and passes every other property straight through. **A field added to `AnalyticsEvent` ships unfiltered, and nothing downstream will catch it.** That is the reason the union must stay a closed, reviewed discriminated union rather than a convenience — reviewing the addition is the whole control. Never sent by this app's code: audio, per-frame pitch/contour data, raw user-agent, precise geolocation, cookies. Country-level geo (`$geoip_country_name`/`code`) is the one deliberate exception, answering a real product question (Taiwan vs. Beijing reference audio); city/region/lat-long are stripped regardless. Marketing events (CTA clicks, `$pageview`) are untouched by the event-name gate. `run_end` carries `voice`, the roster speaker id a run was flown with — an id, never anything the player typed.
...
Accounts, a backend, tier gating and payment are no longer on this list — anonymous auth, the leaderboard, public email accounts with cross-device sync, tier-gated run caps/words/leaderboard, and the Lemon Squeezy entitlement webhook have all shipped (see above). Still not built: **voice-clip storage** (Phase 3 of the original backend arch — not to be confused with the tiers/payment "Phase 3–4" naming above) and **payments or billing of our own** (Lemon Squeezy remains the merchant of record; `api/webhook-ls.ts` only reacts to its webhook, it never talks to a card). **Server-enforced daily limits are now only half true**: a signed-in account's cap is server-authoritative (`api/run.ts`, `daily_runs`); a guest's stays local and stays unenforceable by design, because clearing storage mints a new anonymous identity that no server-side count could follow.
